"""
CLI main entry point for FitLake.

This file provides the main() function that can be called from the
root cli.py or directly via `python -m cli`.

The CLI does NOT import FastAPI - it only uses:
- platforms/ for fetching data from external APIs
- db/ for database operations
- core/ for shared business logic
"""
...
SETUP COMMANDS:
  ...
  sync                Sync all data from the platform (CSV + database)
  sync --db           Sync to database only (no CSV export)
...
EXAMPLES:
  ...
  # Hevy
  python cli.py hevy sync                 # Sync all Hevy data (CSV + DB)
  ...
  python cli.py strava sync               # Sync all Strava activities (CSV + DB)
  python cli.py strava sync --db          # Sync to database only
  ...
  python cli.py garmin sync               # Sync daily stats (default: past 30 days)
  python cli.py garmin sync --days 7      # Sync daily stats (past 7 days)
  python cli.py garmin sync activities    # Sync all activities (CSV + DB)
  python cli.py garmin sync activities --db  # Sync activities to DB only
...
def handle_hevy(args: list[str]):
    ...
    if command == "sync":
        ...
        try:
            ...
            if subcommand is None or subcommand == "--db":
                ...
                print("Usage: python cli.py hevy sync [workouts|templates] [--db]")
